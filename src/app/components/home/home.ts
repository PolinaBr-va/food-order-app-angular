...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
  // Загружаем продукты
    const productsSub = this.productService.getProducts().subscribe(products => {
   ...
   onSearchChange() {
    if (this.searchTerm.trim()) {
      const searchSub = this.productService.searchProducts(this.searchTerm).subscribe(products => {
